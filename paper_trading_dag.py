...
def paper_trading_dag():
    ...
    def create_sell_working_order(symbol, quantity_to_close):
        

        print(f'Creating Sell Working Order for {symbol} with qty {quantity_to_close}')
        ...
        # print('***Sell Order Successfully Submitted!***')
        # # Print the order details
        # print(f"Order ID: {order.id}")
        # print(f"Status: {order.status}")
        # print(f"Symbol: {order.symbol}")
    ...
    def is_order_filled(order_id):
        ...
        if (float(order.filled_qty) >0):
            print(f"Order filled {order.id} of {order.symbol} for {order.order_type}")
            return True
        print(f"Order NOT filled {order.id} of {order.symbol} for {order.order_type}")
        return False
    ...
    def process_allocations_for_removing_them(allocations_to_be_removed: list, allocation_history_status: str, battle_date):
        ...
        for poor_performer in allocations_to_be_removed:
            ...
            if deployment_result:
                ...
                filled = False
                while (filled == False):
                    print("Checking if SELL order is filled yet...")
                    filled = is_order_filled(order.id)
    ...
    def get_active_sectors(battle_date):
        ...
        active_sectors = db_helper.select_all(
            ...
            columns="sector_name",
            where_clause="active_from <= %s AND (active_to >= %s OR active_to IS NULL) AND (status = 'yes' OR status = 'Automated')",
